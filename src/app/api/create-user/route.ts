...
async function verifyAdminRole(
  req: NextRequest
): Promise<{ user: any; isAdmin: boolean }> {
  try {
    ...
    if (roleError || !userData) {
      console.log(
        ...
        roleError?.message || "No user data found"
      );
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      "✅ Verificación exitosa, continuando con creación de usuario..."
    );
    ...
    try {
      ...
      if (!emailResponse.ok) {
        console.warn(
          "⚠️ Usuario creado, pero error al enviar email de bienvenida"
        );
      } else {
